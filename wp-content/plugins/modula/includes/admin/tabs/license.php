		...
		<tr valign="top">
			...
			<td>
				<input id="modula_pro_license_key" name="modula_pro_license_key" type="password" class="regular-text"
					   value="<?php echo esc_attr( $license ); ?>"/><span class="license_activation_status"></span>
				<label class="description modula-license-label" for="modula_pro_license_key">
					...
					if ( '' == $license ){
						...
						if ( 'lifetime' == $status->expires ){
							...
							echo sprintf( '<p class="%s">'.$messages['all-good'].'</p>',$l_stat, $license_expire );

							if ( 'green' != $l_stat ){
								echo sprintf( __( 'You have %s weeks untill your license will expire. Click <a href="%s" target="_blank">here</a> to extend your license.', 'modula-pro' ), $weeks, esc_url( 'https://wp-modula.com/my-account/' ) );
							}
