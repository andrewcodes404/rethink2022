...
class Modula_Video {
    ...
    public function load_plugin_textdomain() {
        load_plugin_textdomain( $this->plugin_slug, false, MODULA_VIDEO_PATH . '/languages/' );
    }
    ...
    public function init() {
        ...
        if( !class_exists( 'Modula_Pro' ) || !get_option('modula_pro_license_status', false) || 'valid' != get_option('modula_pro_license_status', false)->license ) {
    ...
    public function addon_updater( $license_key, $store_url ){
        ...
        if ( class_exists( 'Modula_Pro_Base_Updater' ) ) {
            $modula_addon_updater = new Modula_Pro_Base_Updater( $store_url, MODULA_VIDEO_FILE,
                array(
    	...
    	if ( isset( $item['video_url'] ) && '' != $item['video_url'] ) {
		    ...
		    if( strpos( $item['video_url'], 'vimeo') ) {
			    ...
			    if( strpos( $item['video_url'], 'player') ){
				    ...
				    $vimeo_matcher = "/^.+vimeo.com\/(.*\/)?([\d]+)(.*)?/";
				    preg_match( $vimeo_matcher, $item['video_url'], $matches, PREG_OFFSET_CAPTURE);

				    if(!empty($matches)){

					    $id = $matches[2][0];
					    $start_time = ( '' != $matches[3][0] ) ? $matches[3][0] : '';
				    } else {

					    $request     = wp_remote_get( 'https://vimeo.com/api/oembed.json?url=' . $item['video_url'] );
					    $response    = wp_remote_retrieve_body( $request );
					    $video_array = json_decode( $response, true );
					    $id          = $video_array['video_id'];
					    $start_time = '';
				    }

				    $item['video_url'] = '//player.vimeo.com/video/' . $id . '?hd=1&show_portrait=0&fullscreen=1&' . $start_time ;
    ...
    public function generate_video_css( $css, $gallery_id, $settings ) {
        ...
        if ( isset( $settings['show-video-icon'] ) && $settings['show-video-icon'] ) {
            ...
            if ( isset( $settings['video-icon-size'] ) && 0 == $settings['use-custom-icon'] || 0 == $settings['custom-video-icon'] ) {
                $css .= "#{$gallery_id}  .modula-video-icon { width : " . ( $settings['video-icon-size'] ) . "!important }";
                $css .= "#{$gallery_id} .modula-video-icon > svg{ max-width : 400%!important }";
            }

            if ( isset( $settings['video-icon-size'] ) && 1 == $settings['use-custom-icon'] && 0 != $settings['custom-video-icon'] ) {
                $css .= "#{$gallery_id}  .modula-video-icon { width : " . ( $settings['video-icon-size'] ) . "!important }";
    ...
    /**
     * Add video related settings if license isn't activated.
     *
     * @since 1.0.0
     *
     * @return array $fields.
	...
	public function modula_video_fields(  $fields ){
		...
		$fields['video']['video-license-check'] = array(
			
			"name"        => '',
			"type"        => "content",
			"content"     => esc_html__('In order to use Modula Video addon with your gallery you\'ll need to activate your license ','modula-video').'<a href="edit.php?post_type=modula-gallery&page=modula&modula-tab=licenses">'.esc_html__('here','modula-video').'</a>',
			"priority"    => 1,
    ...
    public function modula_video_tab( $tabs ) {
		...
		$tabs['video']['badge'] = esc_html__( 'Not Activated', 'modula-video' );
    ...
    public static function video_link_formatter( $url ) {
        ...
        // check if vimeo
            ...
            if( strpos( $url, 'player') ){
                ...
                $vimeo_matcher = "/^.+vimeo.com\/(.*\/)?([\d]+)(.*)?/";
                preg_match( $vimeo_matcher, $url, $matches, PREG_OFFSET_CAPTURE);
                ...
                $final_url = '//player.vimeo.com/video/' . $id . '?hd=1&show_portrait=0&fullscreen=1&' . $start_time ;
        ...
        if ( isset( $image['video_url'] ) && '' != $image['video_url'] ) {
	        ...
	        if( strpos( $image_config['src'], 'vimeo') ) {
		        ...
		        if ( !strpos( $image_config['src'], 'player' ) ) {
			        $vimeo_matcher = "/^.+vimeo.com\/(.*\/)?([\d]+)(.*)?/";
			        preg_match( $vimeo_matcher, $image_config['src'], $matches, PREG_OFFSET_CAPTURE );
			        ...
			        if ( !empty( $matches ) ) {
				        ...
				        $request     = wp_remote_get( 'https://vimeo.com/api/oembed.json?url=' . $image_config['src'] );
				        $response    = wp_remote_retrieve_body( $request );
				        $video_array = json_decode( $response, true );
				        $id          = $video_array['video_id'];
				        $start_time  = '';
			        ...
			        $image_config['src'] = '//player.vimeo.com/video/' . $id . '?hd=1&show_portrait=0&fullscreen=1&' . $start_time;
